...
void decideMoviment(int temp) {
	...
	if(indexA == indexB) {
		...
		int *exchange = new int[routes[indexA][vehicles]];
	    for(int k = 0; k < routes[indexA][vehicles]; k++) {
	        exchange[k] = 0;
	    }
	    
	    for(int k = 1; k < routes[indexA][vehicles] - 1; k++) {
	        for(int l = 1; l < routes[indexB][vehicles] - 1; l++) {
